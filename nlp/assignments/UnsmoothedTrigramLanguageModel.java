 ...
 */
class UnsmoothedTrigramLanguageModel implements LanguageModel {
  ...
  String generateNextWord(String word1) {
    double sample = Math.random() * unigramProb(word1);
    double sum = 0.0;
    for (String word2 : unigramCounter.keySet()) {
      sum += condBigramProb(word1, word2);
      if (sum > sample) {
        return word2;
      }
    }
  ...
  public UnsmoothedTrigramLanguageModel(Collection<List<String>> sentenceCollection) {
